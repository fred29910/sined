// Headless smoke test of the Phase 1 architecture. Run with:
//   bun apps/editor/scripts/smoke.ts
...
function count(s: ReturnType<typeof createSampleScene>): number {
  let n = 0;
  s.walk(() => (n += 1));
  return n;
}
...
async function main(): Promise<void> {
  ...
  const events: string[] = [];
  scene.addListener((e) => events.push(e.kind));
  ...
  await bus.execute(new RemoveEntityCommand({ scene, entity: cube }));
  ...
  console.log(`[events] fired ${events.length} scene event(s); kinds: ${[...new Set(events)].join(', ')}`);
}
